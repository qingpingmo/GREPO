...
class EvaluationResultManager:
    ...
    def __init__(self, repo_name: str, cache_dir: str = "./evaluation_cache", graph_data=None, inference_mode: bool = False):
        ...
        logger.info(f"Result Manager Initialization - Repo: {repo_name}, Current Best Hit@10: {self.best_top10_score:.4f}")
    ...
    def _build_node_mapping(self):
        try:
            ...
            dataset_paths = [
                f"savedata/repos/{self.repo_name}/",
                f"Graph_Feature_Construction/savedata/repos/{self.repo_name}/",
                f"./savedata/repos/{self.repo_name}/"
            ]
            ...
            for dataset_path in dataset_paths:
                try:
                    if os.path.exists(dataset_path):
                        ...
                        logger.info(f"The dataset contains {len(textgraphdataset)} nodes")
                        break
                except Exception as e:
                    continue
            
            if textgraphdataset is None:
                logger.warning(f"Unable to find the dataset for {self.repo_name} , attempting an alternative method")
                return
            
            self.node_id_to_info = textgraphdataset
            ...
            logger.info(f"Node mapping completed: {len(self.node_id_to_info)} nodes")
        except Exception as e:
    ...
    def _convert_node_ids_to_info(self, node_ids: List[int]) -> List[Dict]:
        ...
        for node_id in node_ids:
            node_info = self.node_id_to_info[node_id]# if node_id in self.node_id_to_info else None
            if node_info:
            ...
            logger.info(f"Node-ID {node_id} maps to path: {result[-1]['path']}, name: {result[-1]['name']}")
        return result
        ...
        if os.path.exists(self.best_results_file):
            try:
                with open(self.best_results_file, 'r', encoding='utf-8') as f:
                    ...
                    self.best_top10_score = best_data.get('best_hit10_score', 0.0)
