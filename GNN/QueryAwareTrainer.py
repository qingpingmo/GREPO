...
class QueryAwareTrainer:
    ...
    def evaluate(self, test_data, code_embeddings, edge_index, graph_data, max_subgraph_size=60000, 
            ...
            for issue_data in tqdm(test_data, desc="Evaluating"):  
                if True:
                    ...
                    if len(issue_data) == 5:
                        ...
                        issue_idx, queries, anchor_labels, gt_labels = issue_data
                        extra_info = None
        ...
        results = {}
        for k in [1, 5, 10, 20]:
            results[f'gt_top_{k}_hit_rate'] = np.mean(gt_top_k_hits[k]) if gt_top_k_hits[k] else 0.0
        
        
        self.result_manager.cache_evaluation_results(
            test_data=test_data,
        ...
        if not self.args.inference:
            current_top10_score = results['gt_top_10_hit_rate']
            updated_best = self.result_manager.update_best_results_if_better(
                current_top10_score=current_top10_score,
                test_metrics=results
            )
            
            if updated_best:
                logger.info(f"The best results have been updated! New Top 10 score: {current_top10_score:.4f}")
        else:
            
            current_top10_score = results['gt_top_10_hit_rate']
            all_cached_results = self.result_manager._get_all_cached_results()
            
            if all_cached_results:
                inference_results = {
                    'inference_top10_score': current_top10_score,
                ...
                with open(self.result_manager.best_results_file, 'w', encoding='utf-8') as f:
                    json.dump(inference_results, f, indent=2, ensure_ascii=False)
                
                logger.info(f"Inference results saved to: {self.result_manager.best_results_file}")
