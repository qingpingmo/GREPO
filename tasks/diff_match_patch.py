...
class diff_match_patch:
  ...
  def match_main(self, text, pattern, loc):
    ...
    print(f"////match{loc}==", text[loc:loc + len(pattern)], "\n////pattern==", pattern)
    if text == pattern:
  ...
  def patch_apply(self, patches, text):
    ...
    text: str = nullPadding + text + nullPadding
    ...
    for patch in patches:
      previoustext = "\n".join(text.split("\n")[:patch.start2])
      expected_loc = len(previoustext)
      expected_loc = expected_loc + delta
      text1 = self.diff_text1(patch.diffs)
      end_loc = -1
      print("////text1==", patch.start2, expected_loc, previoustext, "\n//currtext", text.split("\n")[patch.start2], "\n//patch text==", text1)
      if False: #len(text1) > self.Match_MaxBits:
